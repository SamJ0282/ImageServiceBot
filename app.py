...
def knowledge_neural(recipient_id):
    message_data = json.dumps({
        ...
        "message": {
            "text": "Neural style transfer is an optimization technique used to take two images—a content image and a style reference image (such as an artwork by a famous painter) and blend them together so the output image looks like the content image, but “painted” in the style of the style reference image."
        }
    })
...
def knowledge_color(recipient_id):
    message_data = json.dumps({
        ...
        "message": {
            "text": "Image colorization is the process of taking an input grayscale (black and white) image and then producing an output colorized image that represents the semantic colors and tones of the input"
        }
    })
...
def received_optin(event):
    ...
    payload = event["optin"]["payload"] #Promblem somewhere here
        ...
        "message":{
            ...
            "quick_replies":[
                ...
                },
                {
                    ...
                    "payload":"postback",#One-time notify problem
                },
                {
                    "content_type":"text",
                    "title":"Neural Image Style",
